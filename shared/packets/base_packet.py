...
class Packet(ABC):
    ...
    def __init__(self, payload: bytes = b''):
        ...
        """
        # TODO: Implement a more thorough validation that lets different packet types hook in for additional requirements.
        if len(payload) > self.MAX_PAYLOAD_LENGTH:
            raise InvalidPayloadError(f"Invalid payload for this packet, exceeds max length. Expected at most {self.MAX_PAYLOAD_LENGTH}B, got {len(payload)}B. Sorry.")
    ...
    def deserialize_header(cls, raw_data: bytes) -> Tuple[PacketType, int]:
        """
            ...
            Args:
                raw_data: Not necessiraly entire packet, but a byte sequence that includes the packet header at the start
            Raises:
        ...
        if(len(raw_data) < cls.HEADER_SIZE):
            raise IncompleteHeaderError(f"The data do not contain a header of a valid length. Expected at least {cls.HEADER_SIZE}B, got {len(raw_data)}B. Sorry.")
    ...
    def _from_payload(cls, payload: bytes):
        ...
        Raises:
            InvalidPayloadError: If the the payload is not valid and the packet can't be reconstructed from that.
        """
        pass
    ...
    def from_payload(cls, payload: bytes):
        ...
        Raises:
            ...
            BaseProtocolError: A generic error has occured during the packet reconstruction.
        """
        if len(payload) > cls.MAX_PAYLOAD_LENGTH:
            raise InvalidPayloadError(f"Invalid payload for this packet, exceeds max length. Expected at most {cls.MAX_PAYLOAD_LENGTH}B, got {len(payload)}B. Sorry.")
        try:
