...
class AsyncioChatProtocol(asyncio.Protocol, EventEmitter):
  ...
  def send_packet(self, packet: Packet):
    ...
    try:
      ...
      raise NetworkError("Something unexpected happend while sending a packet, sorry.") from e
  ...
  async def send_packet_and_wait(self, packet: Packet) -> ResponsePacket:
    """
      ...
      Raises:
        ConnectionClosedError: If the connection was already closed.
        NetworkError: If something unexpected happend during while waiting for response packet.
    """
    if self.is_closed:
      raise ConnectionClosedError("Cant send packet, the connection is closed, sorry.")
  ...
  def _resolve_next_response(self, err: Optional[BaseException], result=None):
    """
      ...
      Parameters:
        ...
        result: A result that the response future will be set to. Ignored when err patameter is not None.
